...
class NSGA2Utils(object):
    ...
    def create_children(self, min_sen, max_sen, population, H, TT_WMD_matrix, MAX_TFIDF_matrix, MAX_TWEET_length_matrix,  max_len_solution) :
        children = []
        solution_no=0
        using_som = 0
        no_using_som = 0
        for individual in population:
            ...
            #flag=1
            MatingPool = random.sample(xrange(0, len(population)), 3) #np.arange(0, len(population)), 1   #mating_generate.mating_pool_generation(H, L, solution_no, neuron_weight,  len(population), beta=0.8)

            mbde=MBDE(MatingPool, population, solution_no, temp1 ,TT_WMD_matrix, MAX_TFIDF_matrix, MAX_TWEET_length_matrix, max_len_solution, b=6, CR=0.8, F=0.8, min_sent_in_summary=min_sen, max_sent_in_summary=max_sen )
            ...
            for p in range(len(new_solutions)):
                    child1 = self.problem.generateIndividual( min_sen, max_sen, max_len_solution , flag=1, sol=new_solutions[p], k=number_of_ones_in_solutions[p])
